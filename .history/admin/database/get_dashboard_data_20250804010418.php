...
try {
    ...
    // 6. Get focus time data by gender from eye tracking sessions
    $focusTimeQuery = "SELECT 
        u.gender,
        AVG(CASE WHEN ets.total_time_seconds > 0 THEN ets.total_time_seconds ELSE NULL END) as avg_focus_time_seconds,
    ...
    $moduleAnalyticsQuery = "SELECT 
        ...
        AVG(CASE 
            ...
            ELSE ets.total_time_seconds 
        END) as avg_time_seconds
        FROM eye_tracking_sessions ets
        JOIN users u ON ets.user_id = u.id
        JOIN modules m ON ets.module_id = m.id
        WHERE u.gender != '' AND (
            ets.focus_time_seconds > 0 OR 
            ets.session_duration_seconds > 0 OR 
            ets.total_time_seconds > 0
        )
