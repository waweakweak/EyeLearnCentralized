<?php
// admin/database/students_minimal.php - Real data API (Simplified)
...
try {
    ...
    if (!$result) {
        echo json_encode(['success' => false, 'error' => 'Query failed: ' . $conn->error, 'query' => $query]);
        exit;
    }
    ...
    while ($row = $result->fetch_assoc()) {
        ...
        $progressQuery = "SELECT AVG(completion_percentage) as avg_progress FROM user_progress WHERE user_id = $student_id";
        ...
        // Get today's focus data for this student
        $focusQuery = "SELECT 
            ...
            SUM(CASE WHEN session_duration_seconds > total_time_seconds 
                ...
                ELSE 0 END) / 60 as unfocus_minutes,
        ...
        if ($focusResult && $focusResult->num_rows > 0) {
            ...
            $today_unfocus_time = round($focusRow['unfocus_minutes'] ?: 0, 0);
            $total_sessions = (int)($focusRow['session_count'] ?: 0);
        }
        
        // If no unfocus time recorded, estimate it
