...
class EyeTrackingService:
    ...
    def run_tracking_loop(self):
        ...
        try:
            while getattr(self, 'is_tracking_enabled', False):
                ...
                # Always process frame (even if None for demo mode)
                # Handle countdown phase
                if self.countdown_active:
                    self.check_countdown()
                    # Generate frame with overlay (demo or real)
                    if frame is not None:
                        frame_with_overlay = self.add_tracking_overlay(frame)
                    else:
                        # Generate demo frame with countdown overlay
        ...
        with self.frame_lock:
            if self.latest_frame is not None:
                ...
                if demo_frame is not None:
                    # Resize and encode demo frame
                    new_width = 320
                    new_height = 240
                    resized_frame = cv2.resize(demo_frame, (new_width, new_height))
    ...
    def generate_demo_frame(self):
        ...
        # Add background
        cv2.rectangle(demo_frame, (10, 10), (310, 230), (30, 30, 30), -1)
        ...
        cv2.putText(demo_frame, "when tracking starts", (50, 130), 
                   cv2.FONT_HERSHEY_SIMPLEX, 0.5, (200, 200, 200), 1)
        ...
        cv2.circle(demo_frame, (x, y), 5, (0, 255, 0), -1)
