    ...
    def start_background_frame_generation(self):
        ...
        def background_frame_loop():
            ...
            while self.frame_generation_enabled:
                try:
                    # Always generate demo frames when not actively tracking with camera
                    if not getattr(self, 'is_tracking_enabled', False) or self.webcam is None:
                        demo_frame = self.generate_demo_frame()
                        with self.frame_lock:
                            self.latest_frame = demo_frame  # Always update for continuous animation
