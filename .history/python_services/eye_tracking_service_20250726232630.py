...
class EyeTrackingService:
            ...
            for camera_index, backend in camera_configs:
                try:
                    ...
                    if self.webcam.isOpened():
                        ...
                        # Test reading frames with more lenient requirements
                        success_count = 0
                        total_tests = 3  # Reduced from 5
                        ...
                        for test_num in range(total_tests):
                            try:
                                ...
                                time.sleep(0.2)  # Give camera time between reads
                            except Exception as read_e:
                                logger.warning(f"❌ Frame test {test_num + 1}/{total_tests}: Exception {read_e}")
                        
                        # Accept camera if at least 2 out of 3 tests succeed
                        if success_count >= 2:
                            logger.info(f"✅ Webcam ACCEPTED! Camera {camera_index}, Backend: {backend}")
                            logger.info(f"🔧 Frame test result: {success_count}/{total_tests} successful reads")
                            return True
                        else:
        ...
        try:
            while getattr(self, 'is_tracking_enabled', False):
                ...
                if not webcam_available or not self.webcam or not self.webcam.isOpened():
                    # Only retry camera every 5 seconds to avoid spam
                    if current_time - last_camera_retry > 5.0 and camera_retry_count < max_camera_retries:
                        logger.info(f"🔄 Attempting camera restart (attempt {camera_retry_count + 1}/{max_camera_retries})...")
                        webcam_available = self.start_webcam()
                        ...
                        if webcam_available:
                            ...
                            logger.warning(f"❌ Camera restart failed (attempt {camera_retry_count}/{max_camera_retries})")
                            self.tracking_state = "camera_retry"
                ...
                if frame is not None:
                    ...
                    # Create status frame when no camera available
                    with self.frame_lock:
    ...
    def get_current_frame_base64(self):
        """Get current frame as base64 string - with status frame fallback"""
        try:
            with self.frame_lock:
                frame_to_use = None
                
                # Check if we have a real camera frame
                if (hasattr(self, 'is_tracking_enabled') and self.is_tracking_enabled and 
                    ...
                    # Create status frame as fallback
...
def get_current_frame():
    """API endpoint to get current annotated frame with tracking info - CAMERA REQUIRED"""
    try:
        frame_data = eye_tracker.get_current_frame_base64()
        if frame_data:
            ...
            return jsonify({
                ...
                'message': 'No camera frame available - real camera required for eye tracking',
                'tracking_state': eye_tracker.tracking_state,
                'timestamp': datetime.now().isoformat()
            }), 400
    except Exception as e:
