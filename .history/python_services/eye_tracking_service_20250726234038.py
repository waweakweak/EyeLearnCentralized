...
class EyeTrackingService:
        ...
        try:
            while getattr(self, 'is_tracking_enabled', False):
                ...
                if not webcam_available or not self.webcam or not self.webcam.isOpened():
                    # Only retry camera every 10 seconds to avoid spam and reduce max retries
                    if current_time - last_camera_retry > 10.0 and camera_retry_count < max_camera_retries:
                    ...
                    time.sleep(1.0)  # Longer sleep when no camera to reduce CPU usage
    ...
    def start_tracking(self, user_id, module_id, section_id=None):
        ...
        # Stop any existing tracking first to avoid conflicts
        if getattr(self, 'is_tracking_enabled', False):
            logger.info("Stopping existing tracking session before starting new one")
            self.stop_tracking()
...
frame_request_interval = 0.1  # Minimum 100ms between frame requests
