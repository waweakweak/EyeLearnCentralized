...
def health_check():
    ...
    try:
        ...
        return jsonify({
            ...
            'tracking_active': getattr(eye_tracker, 'is_tracking_enabled', False),
            'current_user_id': getattr(eye_tracker, 'current_user_id', None),
            'current_module_id': getattr(eye_tracker, 'current_module_id', None),
            'current_section_id': getattr(eye_tracker, 'current_section_id', None),
            'timestamp': datetime.now().isoformat()
        })
    except Exception as e:
        return jsonify({'success': False, 'error': str(e)}), 500
