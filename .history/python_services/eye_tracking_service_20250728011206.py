...
class EyeTrackingService:
    ...
    def start_countdown(self):
        ...
        logger.info("🕒 Starting 3-second countdown for NEW MODULE or INITIAL TRACKING...")
    ...
    def start_tracking(self, user_id, module_id, section_id=None):
        ...
        logger.info(f"start_tracking called: user_id={user_id}, module_id={module_id}, section_id={section_id}")
        logger.info(f"Current state: tracking_enabled={self.is_tracking_enabled}, current_user={getattr(self, 'current_user_id', None)}, current_module={getattr(self, 'current_module_id', None)}, current_section={getattr(self, 'current_section_id', None)}")
        
        # Check if this is exactly the same request (duplicate)
        ...
        if is_same_request:
            logger.info("🔄 Duplicate request - tracking already active for exact same user/module/section")
            return
        
        # Check if this is a section change within the same module/user
        # This should NOT trigger countdown
        is_section_change = (self.is_tracking_enabled and 
                            self.current_user_id == user_id and 
                            self.current_module_id == module_id and
                            self.current_section_id != section_id)
        
        # Only stop if different user/module
        if (self.is_tracking_enabled and 
        ...
        # Update current tracking parameters
        ...
        # If we reach here, this is a NEW MODULE or FIRST TIME tracking
        logger.info(f"🆕 NEW MODULE or FIRST TIME - will start countdown")
        ...
        logger.info(f"🚀 Starting countdown for NEW MODULE: user {user_id}, module {module_id}")
