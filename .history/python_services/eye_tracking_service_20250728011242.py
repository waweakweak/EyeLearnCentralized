...
class EyeTrackingService:
    ...
    def start_tracking(self, user_id, module_id, section_id=None):
        ...
        logger.info(f"Current state: tracking_enabled={getattr(self, 'is_tracking_enabled', False)}, current_user={getattr(self, 'current_user_id', None)}, current_module={getattr(self, 'current_module_id', None)}, current_section={getattr(self, 'current_section_id', None)}")
        ...
        # Check if tracking is already active for exactly the same parameters
        ...
        if is_same_request:
            logger.info("Tracking already active for exact same user/module/section - ignoring duplicate request")
            return
