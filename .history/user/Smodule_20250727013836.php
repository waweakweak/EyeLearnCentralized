...
<head>
    ...
    <link rel="stylesheet" href="/src/output.css">
        ...
        <script src="js/cv-eye-tracking.js?canvas_stability_fix_<?php echo time(); ?>"></script>
    <script>
        tailwind.config = {
            theme: {
                extend: {
...
<div class="mb-8">
    ...
    function startEyeTrackingForModule(moduleId, moduleTitle) {
        console.log(`🚀 Starting eye tracking for module: ${moduleTitle} (ID: ${moduleId})`);
        
        // Stop any existing tracking
        if (window.cvEyeTracker && window.cvEyeTracker.isTracking) {
            window.cvEyeTracker.stopTracking();
        }
        
        // Clear any previous countdown session for this module
        const sessionKey = `eyetracking_countdown_${moduleId}`;
        sessionStorage.removeItem(sessionKey);
        
        // Create new eye tracking instance which will trigger countdown
        setTimeout(() => {
            try {
                window.cvEyeTracker = new CVEyeTrackingSystem(moduleId, 'main_content');
                window.cvEyeTracker.moduleTitle = moduleTitle;
                console.log(`✅ Eye tracking started for: ${moduleTitle}`);
            } catch (error) {
