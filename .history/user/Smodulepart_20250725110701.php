...
// Initialize CV Eye Tracking System for module content
console.log('🎯 Initializing CV Eye Tracking for Module Content...');
try {
    if (typeof CVEyeTrackingSystem !== 'undefined') {
        // Get module and section IDs from PHP variables
        const moduleId = <?php echo json_encode($selected_module_id); ?>;
        const sectionId = <?php echo json_encode($selected_section_id); ?>;
        
        console.log('📊 Module ID:', moduleId, 'Section ID:', sectionId);
        
        // Initialize CV eye tracking with actual IDs
        window.cvEyeTracker = new CVEyeTrackingSystem(moduleId, sectionId);
        console.log('✅ CV Eye Tracking initialized for module content');
        ...
        console.log('⚠️ CV Eye Tracking not available - script may not have loaded');
    }
} catch (error) {
    console.error('❌ Failed to initialize CV Eye Tracking:', error);
}
    </script>
</body> 
</html>