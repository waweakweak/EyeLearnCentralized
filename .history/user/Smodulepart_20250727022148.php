...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function startModuleEyeTracking() {
        console.log(`🚀 Starting CVEyeTrackingSystem for module part: ${moduleTitle} (ID: ${moduleId}) Section: ${sectionId}`);
        
        try {
            // Check if eye tracking system is available
            if (typeof CVEyeTrackingSystem !== 'undefined') {
                // Stop any existing simple tracking first
                if (window.moduleFrameUpdateInterval) {
                    clearInterval(window.moduleFrameUpdateInterval);
                    window.moduleFrameUpdateInterval = null;
                }
                
                window.cvEyeTracker = new CVEyeTrackingSystem(moduleId, 'main_content');
                window.cvEyeTracker.moduleTitle = moduleTitle;
                window.cvEyeTracker.sectionId = sectionId;
                console.log(`✅ CVEyeTrackingSystem started for module content: ${moduleTitle}`);
            } else {
                console.warn('⚠️ Eye tracking system not available');
            }
        } catch (error) {
            console.error('❌ Failed to start module eye tracking:', error);
        }
    }
