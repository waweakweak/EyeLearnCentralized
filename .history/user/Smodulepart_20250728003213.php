...
<body class="bg-background">
    ...
    <div class="flex min-h-screen pt-16">
        ...
        <div id="live-feed-container" style="position: fixed; top: 80px; right: 20px; z-index: 1000; background: white; border: 1px solid #ddd; border-radius: 6px; padding: 8px; box-shadow: 0 2px 8px rgba(0,0,0,0.15); display: none;">
            <img id="tracking-video" alt="Live camera feed" style="width: 160px; height: 120px; background-color: #000; border-radius: 4px; display: block;">
        </div>
...
document.addEventListener('DOMContentLoaded', function() {
        ...
        try {
            ...
            if (response.ok && responseData.success) {
                ...
                if (feedContainer) {
                    ...
                    console.error('❌ Could not find live-feed-container element');
                }
                
                // Start frame updates exactly like the test (100ms interval)
                moduleFrameUpdateInterval = setInterval(updateModuleVideoFrame, 100);
                console.log('🔄 Frame update interval started (100ms)');
            } else {
                throw new Error(`Server responded with error: ${responseData.error || 'Unknown error'}`);
            }
        } catch (error) {
            console.log(`❌ Failed to start module part tracking: ${error.message}`);
        }
    }
    ...
    async function stopModuleTracking() {
        try {
            ...
            if (response.ok) {
                console.log('✅ Module part tracking stopped');
            } else {
                throw new Error(`HTTP ${response.status}`);
            }
        } catch (error) {
            console.log(`❌ Failed to stop module part tracking: ${error.message}`);
        }
    }

    // Function to start eye tracking for module content
    async function startModuleEyeTracking() {
        console.log(`🚀 Starting simple eye tracking for module part: ${moduleTitle} (ID: ${moduleId}) Section: ${sectionId}`);
        
        // Check if tracking is already active for this module
        const currentTrackingModule = sessionStorage.getItem('activeTrackingModule');
        const currentTrackingUser = sessionStorage.getItem('activeTrackingUser');
        const userId = <?php echo json_encode($user_id); ?>;
        
        if (currentTrackingModule === moduleId && currentTrackingUser === userId.toString()) {
            console.log('✅ Eye tracking already active for this module - continuing without restart');
            
            // Check if backend is still tracking this module
            try {
                const healthResponse = await fetch(`${MODULE_API_BASE}/health`);
                if (healthResponse.ok) {
                    const healthData = await healthResponse.json();
                    if (healthData.tracking_active && 
                        healthData.current_module_id == moduleId && 
                        healthData.current_user_id == userId) {
                        
                        console.log('🔄 Backend tracking confirmed active - just updating frames');
                        
                        // Show the live feed container if not visible
                        const feedContainer = document.getElementById('live-feed-container');
                        if (feedContainer) {
                            feedContainer.style.display = 'block';
                        }
                        
                        // Just start frame updates without calling backend start_tracking
                        if (!moduleFrameUpdateInterval) {
                            moduleFrameUpdateInterval = setInterval(updateModuleVideoFrame, 100);
                            console.log('🔄 Frame update interval started (100ms) - continuing existing session');
                        }
                        return;
                    }
                }
            } catch (error) {
                console.log('⚠️ Health check failed, will restart tracking:', error.message);
            }
        }
        
        // Stop any existing tracking for different module/user
        if (moduleFrameUpdateInterval) {
            stopModuleTracking();
        }
        
        // Store the current module as the active tracking module
        sessionStorage.setItem('activeTrackingModule', moduleId);
        sessionStorage.setItem('activeTrackingUser', userId.toString());
        
        // Start tracking with the simple approach that works (this will trigger countdown for new modules)
        setTimeout(() => {
            startModuleTracking();
        }, 100);
    }
    ...
    // Clear tracking session when user exits (optional - for cleanup)
    window.addEventListener('beforeunload', function() {
        if (moduleFrameUpdateInterval) {
            sessionStorage.removeItem('activeTrackingModule');
            sessionStorage.removeItem('activeTrackingUser');
        }
    });
    
    // Start eye tracking after a short delay to ensure page is fully loaded
    setTimeout(startModuleEyeTracking, 1000);
});
    </script>
</body> 
</html>