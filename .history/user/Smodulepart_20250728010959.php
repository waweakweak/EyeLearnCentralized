...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function startModuleEyeTracking() {
        ...
        // If same user, module, and section - just continue with frame updates
        ...
        // If same user and module but different section - just update section and continue
        if (currentTrackingModule === moduleId && 
            currentTrackingUser === userId.toString() && 
            currentTrackingSection !== sectionId) {
            ...
            // Send section change to backend
            startModuleTracking();
            
            // Continue frame updates
            if (!moduleFrameUpdateInterval) {
                moduleFrameUpdateInterval = setInterval(updateModuleVideoFrame, 100);
                console.log('🔄 Frame update interval started (100ms) - section change');
            }
            return;
        }
        ...
        // Start tracking immediately - no artificial delay needed
        startModuleTracking();
    }
    ...
    window.addEventListener('beforeunload', function() {
        if (moduleFrameUpdateInterval) {
            ...
            sessionStorage.removeItem('activeTrackingSection');
        }
    });
    
    // Start eye tracking immediately when page loads - no delay needed
    startModuleEyeTracking();
});
    </script>
</body> 
</html>