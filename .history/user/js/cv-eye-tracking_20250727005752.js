                ...
                if (data.success && data.hasFrame === true && data.frameData && data.frameData.length > 0) {
                    ...
                    if (videoElement) {
                        ...
                        videoElement.onload = () => {
                            ...
                            console.log('✅ Video frame loaded and displayed successfully');
                        ...
                        videoElement.onerror = (error) => {
                            ...
                            console.log('🔍 Failed frame data prefix:', data.frameData.substring(0, 50));
                        };
                        
                        // Set the frame data
                        try {
                            videoElement.src = data.frameData;
                            console.log('✅ Frame data assigned to video element');
                            
                            // Additional debugging - check if src was actually set
                            setTimeout(() => {
                                console.log('🔍 Video element src after assignment:', {
                                    srcLength: videoElement.src ? videoElement.src.length : 0,
                                    srcPrefix: videoElement.src ? videoElement.src.substring(0, 30) : 'none',
                                    complete: videoElement.complete,
                                    naturalWidth: videoElement.naturalWidth,
                                    naturalHeight: videoElement.naturalHeight
                                });
                                ...
                                if (videoElement.naturalWidth === 0) {
                                    console.warn('⚠️ Video element has no natural dimensions - image may not have loaded');
                                    // Try to trigger a reload
                                    const currentSrc = videoElement.src;
                                    videoElement.src = '';
                                    setTimeout(() => {
                                        videoElement.src = currentSrc;
                                        console.log('🔄 Attempted to reload video element');
                                    }, 10);
                                }
                            }, 100);
                            
                        } catch (error) {
                            console.error('❌ Error assigning frame data:', error);
                        }
                    } else {
                        console.error('❌ Video element with ID "tracking-video" not found!');
