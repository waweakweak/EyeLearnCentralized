...
class CVEyeTrackingSystem {
    ...
    startVideoUpdates() {
        ...
        }, 50); // Update every 50ms for 20 FPS (was 67ms = 15 FPS)
        
        console.log('✅ Video update interval started (50ms = 20 FPS with flicker protection)');
    }
            ...
            if (response.ok) {
                ...
                if (data.success && data.hasFrame === true && data.frameData && data.frameData.length > 0) {
                    ...
                    if (canvasElement) {
                        ...
                        console.log('🔍 Found video/img elements:', allVideos.length);
    ...
    initializeFrameBuffer(canvasElement) {
        ...
        this.frameBuffer.context = canvasElement.getContext('2d');
        ...
        this.frameBuffer.context.imageSmoothingEnabled = true;
        this.frameBuffer.context.imageSmoothingQuality = 'high';
    ...
    drawFrameToCanvas(frameData, canvasElement) {
        ...
        // Prevent too frequent updates that cause flickering
        const timeSinceLastFrame = now - this.frameBuffer.lastFrameTime;
        if (timeSinceLastFrame < 50) { // Minimum 50ms between frames (20 FPS max)
            return; // Skip this frame to prevent flicker
        }
        
        // Enhanced debugging for canvas issues
        ...
        // Create image only when needed and reuse context
        const img = new Image();
        img.onload = () => {
            try {
                ...
                // Direct drawing without requestAnimationFrame first
                try {
                    // Use more efficient drawing - only clear the area we need
                    ...
                    console.log('✅ Frame drawn directly to canvas:', {
                ...
                if (this.frameBuffer.frameCount % 300 === 0) { // Every 300 frames (about 20 seconds at 15fps)
                ...
                console.error('❌ Optimized canvas drawing error:', error);
            }
        };
