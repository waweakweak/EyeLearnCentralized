...
class CVEyeTrackingSystem {
    ...
    startVideoUpdates() {
        ...
        // Use a slightly slower frequency for debugging image loading issues
        this.videoUpdateInterval = setInterval(async () => {
            await this.updateVideoFrame();
        }, 500); // Update every 500ms for 2 FPS - slower for debugging
        
        console.log('✅ Video update interval started (500ms = 2 FPS - slower for debugging)');
    }
    ...
    // SIMPLE updateVideoFrame method - using the same approach as the working test
        ...
        try {
            ...
            if (response.ok) {
                ...
                if (data.hasFrame && data.frameData) {
                    const videoElement = document.getElementById('tracking-video');
                    if (videoElement) {
                        console.log('🖼️ Setting new frame:', {
                            elementExists: !!videoElement,
                            currentSrc: videoElement.src ? videoElement.src.substring(0, 50) + '...' : 'none',
                            newDataLength: data.frameData.length,
                            newDataPrefix: data.frameData.substring(0, 50) + '...'
                        });
                        
                        // Simple direct assignment - just like the test file
                        videoElement.src = data.frameData;
                        
                        // Add onload and onerror handlers to debug image loading
                        videoElement.onload = () => {
                            console.log(`✅ Image loaded successfully (${data.frameData.length} chars)`);
                        };
                        
                        videoElement.onerror = (error) => {
                            console.error('❌ Image failed to load:', error);
                            console.log('Frame data details:', {
                                length: data.frameData.length,
                                prefix: data.frameData.substring(0, 50),
            ...
            if (error.name === 'TypeError' && error.message.includes('fetch')) {
                // This is likely a connection error - handle gracefully
                this.handleServiceDisconnection();
            } else {
                // Other errors - reduce logging frequency
                if (Math.random() < 0.02) { // 2% chance
