...
class CVEyeTrackingSystem {
    ...
    startVideoUpdates() {
        ...
        // Faster updates for truly smooth live feed
        this.videoUpdateInterval = setInterval(async () => {
            await this.updateVideoFrame();
        }, 50); // Update every 50ms for 20 FPS - faster for smoother live feed
        
        console.log('✅ Video update interval started (50ms = 20 FPS - faster for smooth live feed)');
    ...
    startVideoWatchdog() {
        // Check every 3 seconds if video is still updating
        this.videoWatchdog = setInterval(() => {
            const videoElement = document.getElementById('tracking-video');
            if (videoElement && this.isConnected) {
                // Check if video element is still visible and functional
                ...
                if (timeSinceLastFrame > 5000 && this.frameCount > 0) { // No frames for 5 seconds
                    console.warn('⚠️ No frames received for 5 seconds - restarting video feed...');
                    this.frameCount = 0;
                    this.consecutiveFrameFailures = 0;
                    this.stopVideoUpdates();
                    this.startVideoUpdates();
                } else if (this.consecutiveFrameFailures > 20) { // Too many failures
                    console.warn('⚠️ Too many consecutive frame failures - restarting video feed...');
                    this.consecutiveFrameFailures = 0;
                    this.stopVideoUpdates();
                    this.startVideoUpdates();
                }
    ...
    // ROBUST updateVideoFrame method - ensures continuous live feed
        ...
        try {
            ...
            if (response.ok) {
                ...
                if (data.hasFrame && data.frameData) {
                    const videoElement = document.getElementById('tracking-video');
                    if (videoElement) {
                        // Track frame continuity
                        this.frameCount++;
                        this.lastFrameTime = Date.now();
                        this.consecutiveFrameFailures = 0; // Reset failure count
                        
                        // Direct assignment for immediate display
                        videoElement.src = data.frameData;
                        
                        // Add a timestamp parameter to prevent browser caching
                        if (videoElement.src === data.frameData) {
                            videoElement.src = data.frameData + '#' + Date.now();
                        }
                        
                        // Log frame statistics periodically
                        if (this.frameCount % 100 === 0) { // Every 100 frames
                            console.log(`📊 Live feed stats: ${this.frameCount} frames processed, last frame: ${data.frameData.length} chars`);
                        }
                    }
                } else {
                    this.consecutiveFrameFailures++;
                    if (this.consecutiveFrameFailures % 10 === 0) { // Every 10 failures
                        console.warn(`⚠️ ${this.consecutiveFrameFailures} consecutive frame failures`);
                    }
                }
