...
class CVEyeTrackingSystem {
    ...
    switchSection(newSectionId) {
        ...
        // Only update the backend if the service is already running and connected
        if (this.isConnected && this.isTracking) {
            console.log(`📡 Notifying backend of section change: ${oldSectionId} → ${newSectionId}`);
            // The backend will handle this as a section change (no service restart)
            this.startTracking();
        } else {
            console.log(`🔄 Service not running, section updated to ${newSectionId}`);
        }
        
        console.log(`✅ Section switched to ${newSectionId} - tracking continues seamlessly`);
    }
