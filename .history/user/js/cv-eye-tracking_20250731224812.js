...
class CVEyeTrackingSystem {
    ...
    handleServiceDisconnection() {
        if (this.isConnected) {
            ...
            // Don't stop video updates immediately - let them continue trying
            // Only reconnect after a longer delay to avoid spam
            setTimeout(async () => {
                await this.checkServiceHealth();
                if (this.isConnected) {
                    console.log('🔌 Service reconnected successfully');
    ...
    // Method to switch to a new section within the same module
    async switchSection(newSectionId) {
        ...
        // Only switch if service is running and it's actually a different section
        if (!this.isConnected || !this.isTracking) {
            console.log(`� Service not running, just updating section ID to ${newSectionId}`);
        ...
        try {
            ...
            if (result.success) {
                this.sectionId = newSectionId;
                console.log(`✅ Section switched successfully: ${oldSectionId} → ${newSectionId} (tracking continues)`);
            } else {
                console.error('❌ Section switch failed:', result.error);
                // Fallback to regular start tracking
                this.sectionId = newSectionId;
                await this.startTracking();
            }
    ...
    // Static method to handle section changes across page navigations
    static async handleSectionChange(moduleId, newSectionId) {
        ...
        if (window.cvEyeTracker && window.cvEyeTracker.moduleId === moduleId) {
            ...
            // This shouldn't happen in normal navigation, but handle it gracefully
            console.log('⚠️ No existing tracker found, creating new one');
