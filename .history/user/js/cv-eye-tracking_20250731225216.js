...
class CVEyeTrackingSystem {
    ...
    showCountdownNotification() {
        // Create compact centered countdown overlay - VISUAL ONLY (services already running)
        ...
        countdownOverlay.innerHTML = `
            <div class="bg-gray-800 text-white rounded-lg shadow-2xl p-6 text-center" style="width: 220px; height: 220px; display: flex; flex-direction: column; justify-content: center; align-items: center;">
                ...
                <!-- Status Text -->
                <div class="text-xs text-green-300" id="countdown-status">
                    Services Active!
                </div>
            </div>
        `;
        ...
        // Start countdown sequence: 3, 2, 1, rocket (VISUAL ONLY - services already running)
        ...
        statusText.textContent = 'Services Active!';
        ...
        const countdownInterval = setInterval(() => {
            ...
            if (secondsRemaining > 0) {
                ...
                countdownNumber.textContent = secondsRemaining;
                statusText.textContent = 'Services Active!';
                console.log(`⏱️ Countdown display: ${secondsRemaining} (services already running)`);
            } else {
                // Show rocket and launch message
                console.log('🚀 Countdown display complete - services fully ready');
...
// Initialize CV eye tracking when DOM is loaded - ENHANCED WITH ERROR HANDLING
document.addEventListener('DOMContentLoaded', async function() {
    try {
        ...
        if (moduleId) {
            ...
            // Check if we have an existing tracker for the same module
            if (window.cvEyeTracker && window.cvEyeTracker.moduleId === moduleIdInt) {
                console.log('🔄 Existing tracker found - switching section safely...');
                
                try {
                    // Update section without stopping the service
                    await window.cvEyeTracker.switchSection(sectionIdInt);
                } catch (error) {
                    console.error('❌ Section switch failed, creating new tracker:', error);
                ...
                if (window.cvEyeTracker) {
                    try {
                        ...
                        console.warn('⚠️ Error cleaning up previous tracker:', error);
                    }
                }
                
                // Initialize new CV eye tracking for different module
                window.cvEyeTracker = new CVEyeTrackingSystem(moduleIdInt, sectionIdInt);
            }
            
            console.log('✅ CV Eye tracking system ready for module:', moduleId, 'section:', sectionId);
            
            // Handle page unload with error handling
