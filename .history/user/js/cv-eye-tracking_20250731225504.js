...
class CVEyeTrackingSystem {
    ...
    // Static method to handle section changes across page navigations - ENHANCED
    static async handleSectionChange(moduleId, newSectionId) {
        ...
        try {
            // If there's an existing tracker for the same module, switch section
            if (window.cvEyeTracker && window.cvEyeTracker.moduleId === moduleId) {
                console.log('🔄 Existing tracker found, performing robust section switch...');
                await window.cvEyeTracker.switchSection(newSectionId);
            } else {
                // Different module or no existing tracker
                console.log('🆕 Creating new tracker for different module or first load');
                
                // Clean up existing tracker if it exists
                if (window.cvEyeTracker) {
                    console.log('🧹 Cleaning up previous tracker...');
                    try {
                        await window.cvEyeTracker.stopTracking();
                    } catch (error) {
                        console.warn('⚠️ Error stopping previous tracker:', error);
                    }
                }
            ...
            // Fallback: try to create a new tracker
            try {
                console.log('🔄 Fallback: creating new tracker...');
                window.cvEyeTracker = new CVEyeTrackingSystem(moduleId, newSectionId);
            } catch (fallbackError) {
