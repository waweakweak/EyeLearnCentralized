/**
 * Enhanced Computer Vision Eye Tracking System v2.2
 * Features: Optimized instant activation, crash-resistant section switching, health monitoring
 * OPTIMIZATIONS:
 * - Removed 3-second delay: Services start immediately, countdown is visual-only
 * - Added continuous health monitoring with automatic reconnection
 * - Robust section switching with multiple fallback strategies
 * - Enhanced error handling to prevent crashes during navigation
 * - Proper cleanup of all intervals and resources
 */
...
class CVEyeTrackingSystem {
    ...
    async init() {
        console.log('🎯 Initializing Enhanced CV Eye Tracking System v2.2...');
        console.log('Features: Optimized countdown, instant activation, crash-resistant section switching');
        ...
        if (this.isConnected) {
            ...
            if (shouldShowCountdown) {
                console.log('🎬 New module - starting countdown for module:', this.moduleId);
                
                // Mark countdown as shown
                this.markCountdownShownForModule();
                
                // Show countdown notification first for proper UX
                this.showCountdownNotification();
                
                // Start services immediately in parallel with countdown display
                console.log('🚀 Starting services while countdown is showing...');
                await this.startTracking();
                this.setupStatusUpdates();
                this.displayTrackingInterface();
                this.initializeTimers();
                
                console.log('✅ Eye tracking services ready during countdown - will be fully operational when countdown ends');
            } else {
                console.log('📝 Section change - starting tracking immediately for module:', this.moduleId);
                
                // Start tracking immediately (no countdown for section changes)
                await this.startTracking();
                this.setupStatusUpdates();
                this.displayTrackingInterface();
                this.initializeTimers();
                console.log('✅ Eye tracking activated immediately (section change)');
            }
    ...
    async stopTracking() {
        ...
        if (!this.isConnected || !this.isTracking) {
            console.log('ℹ️ Service not connected or not tracking, cleaning up anyway...');
        }

        // Try to stop tracking on the service
        if (this.isConnected && this.isTracking) {
            try {
        ...
        // Always clean up local state and intervals regardless of service response
        this.isTracking = false;
        this.countdownActive = false;
        this.isConnected = false; // Force disconnect to prevent further operations
        ...
        console.log('✅ Eye tracking stopped and cleaned up completely');
    }
    ...
    // SIMPLIFIED updateVideoFrame method - EXACTLY like the working test
        ...
        try {
            ...
            if (response.ok) {
                ...
                if (data.hasFrame && data.frameData) {
                    ...
                    if (Math.random() < 0.1) { // 10% chance to avoid spam - same as test
    ...
    // Method to switch to a new section within the same module - CRASH-RESISTANT
    async switchSection(newSectionId) {
        ...
        // Strategy 2: If API failed, try graceful restart
        if (!switchSuccess && this.isConnected) {
            console.log('🔄 API switch failed, attempting graceful restart...');
            
            try {
                // Don't stop tracking completely, just restart with new section
                const userId = await this.getCurrentUserId();
        ...
        if (!switchSuccess) {
            ...
            if (this.isConnected) {
                ...
                // Service is down - trigger reconnection
                console.warn('❌ Service appears down, triggering reconnection...');
                await this.attemptReconnection();
            }
        }
        
        console.log(`🔄 Section switch completed: ${oldSectionId} → ${newSectionId} (success: ${switchSuccess})`);
    }
    
    // Static method to handle section changes across page navigations - ENHANCED FOR MODULE SWITCHING
    static async handleSectionChange(moduleId, newSectionId) {
        ...
        try {
            // Check if we have an existing tracker
            if (window.cvEyeTracker) {
                if (window.cvEyeTracker.moduleId === moduleId) {
                    // Same module - switch section
                    console.log('🔄 Same module, performing section switch...');
                    await window.cvEyeTracker.switchSection(newSectionId);
                } else {
                    // Different module - clean up and create new tracker
                    console.log(`🔄 Different module (${window.cvEyeTracker.moduleId} → ${moduleId}), replacing tracker...`);
                    
                    // Stop and clean up existing tracker
                    try {
                        await window.cvEyeTracker.stopTracking();
                        ...
                        console.log('✅ Previous tracker cleaned up');
                    } catch (error) {
                        console.warn('⚠️ Error during cleanup:', error);
                        window.cvEyeTracker = null; // Force clear
                    }
                    
                    // Create new tracker for different module
                    window.cvEyeTracker = new CVEyeTrackingSystem(moduleId, newSectionId);
                    console.log(`✅ New tracker created for module ${moduleId}`);
                }
            } else {
                // No existing tracker - create new one
                console.log('🆕 No existing tracker, creating new one...');
                window.cvEyeTracker = new CVEyeTrackingSystem(moduleId, newSectionId);
            }
        } catch (error) {
            console.error('❌ Error in static section change handler:', error);
            
            // Fallback: Force clean slate and create new tracker
            try {
                console.log('🔄 Fallback: Force cleanup and create new tracker...');
                ...
                if (window.cvEyeTracker) {
                    try {
                        ...
                        console.warn('⚠️ Error during force cleanup:', cleanupError);
                    }
                }
                ...
                // Clear reference and create new
                window.cvEyeTracker = null;
                window.cvEyeTracker = new CVEyeTrackingSystem(moduleId, newSectionId);
                console.log('✅ Fallback tracker created successfully');
...
// Initialize CV eye tracking when DOM is loaded - ENHANCED WITH BETTER MODULE SWITCHING
document.addEventListener('DOMContentLoaded', async function() {
    try {
        ...
        if (moduleId) {
            ...
            // Check if we have an existing tracker
            if (window.cvEyeTracker) {
                if (window.cvEyeTracker.moduleId === moduleIdInt) {
                    // Same module - just switch section
                    console.log('🔄 Same module, switching section safely...');
                    
                    try {
                        await window.cvEyeTracker.switchSection(sectionIdInt);
                        console.log('✅ Section switch completed successfully');
                        return; // Exit early - no need to create new tracker
                    } catch (error) {
                        console.error('❌ Section switch failed:', error);
                        // Fall through to create new tracker
                    }
                } else {
                    // Different module - need to stop old tracker and create new one
                    console.log(`🔄 Different module detected (${window.cvEyeTracker.moduleId} → ${moduleIdInt}), creating new tracker...`);
                    
                    try {
                        // Properly stop the existing tracker
                        console.log('🛑 Stopping previous tracker...');
                        await window.cvEyeTracker.stopTracking();
                        ...
                        if (existingInterface) {
                            existingInterface.remove();
                            console.log('🗑️ Removed previous interface');
                        }
                        
                        // Clear the reference
                        window.cvEyeTracker = null;
                        console.log('✅ Previous tracker cleaned up successfully');
                        
                    } catch (error) {
                        console.warn('⚠️ Error during cleanup, proceeding anyway:', error);
                        window.cvEyeTracker = null; // Force clear
                    }
                }
            }
            
            // Create new tracker (either first time or after cleanup)
            console.log(`🆕 Creating new tracker for module: ${moduleIdInt}, section: ${sectionIdInt}`);
            
            try {
                window.cvEyeTracker = new CVEyeTrackingSystem(moduleIdInt, sectionIdInt);
                console.log('✅ New tracker created successfully');
            } catch (error) {
                console.error('❌ Failed to create new tracker:', error);
                
                // Try one more time with a slight delay
                setTimeout(() => {
                    try {
                        window.cvEyeTracker = new CVEyeTrackingSystem(moduleIdInt, sectionIdInt);
                        console.log('✅ New tracker created successfully (retry)');
                    } catch (retryError) {
                        console.error('❌ Failed to create tracker even on retry:', retryError);
                    }
                }, 1000);
            }
