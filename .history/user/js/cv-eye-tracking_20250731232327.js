...
class CVEyeTrackingSystem {
    ...
    async stopTracking() {
        ...
        // Set transitioning flag to prevent video update errors (only if this is a transition)
        ...
        // Try to stop tracking on the service - ALWAYS send stop signal
        if (this.isConnected && this.isTracking) {
            try {
                ...
                if (response.ok) {
                    ...
                    console.warn('⚠️ Service responded with error when stopping tracking');
                }
            } catch (error) {
                console.warn('⚠️ Error stopping tracking on service:', error);
                
                // Fallback: Try to send stop signal without waiting for response
                try {
                    ...
                    console.warn('⚠️ Fallback stop signal also failed:', fallbackError);
                }
            }
        }
...
// Add global function to manually shut down eye tracking (can be called by exit buttons)
window.shutdownEyeTracking = function() {
    console.log('🚪 Manual eye tracking shutdown triggered...');
    try {
        if (window.cvEyeTracker) {
            // Force complete shutdown
            window.cvEyeTracker.isTransitioning = false;
            
            // Stop service completely
            if (window.cvEyeTracker.isTracking) {
                window.cvEyeTracker.stopTracking().catch(error => {
                    console.warn('⚠️ Error stopping tracking on manual shutdown:', error);
                });
            }
            ...
            window.cvEyeTracker.stopHealthMonitoring();
            
            // Send explicit stop signal to service
            if (window.cvEyeTracker.isConnected) {
                fetch(`${window.cvEyeTracker.pythonServiceUrl}/api/stop_tracking`, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    keepalive: true
                }).catch(error => {
                    console.warn('⚠️ Error sending stop signal to service on manual shutdown:', error);
                });
            }
            
            // Clear global reference
            window.cvEyeTracker = null;
            console.log('✅ Eye tracking service shut down completely (manual)');
        } else {
            console.log('ℹ️ No eye tracking service to shut down');
        }
    } catch (error) {
        console.warn('⚠️ Error during manual shutdown:', error);
    }
};
    ...
    try {
        ...
        if (moduleId) {
            ...
            if (!window.eyeTrackingUnloadHandlerAdded) {
                ...
                // Handle course exit - detect when user navigates away from course
                const handleCourseExit = () => {
                    try {
                        console.log('🚪 Course exit detected - shutting down eye tracking service...');
                        if (window.cvEyeTracker) {
                            // Force complete shutdown (not just transitioning)
                            window.cvEyeTracker.isTransitioning = false;
                            
                            // Stop service completely
                            if (window.cvEyeTracker.isTracking) {
                                window.cvEyeTracker.stopTracking().catch(error => {
                                    console.warn('⚠️ Error stopping tracking on course exit:', error);
                                });
                            }
                            ...
                            if (window.cvEyeTracker.isConnected) {
                                fetch(`${window.cvEyeTracker.pythonServiceUrl}/api/stop_tracking`, {
                                    ...
                                    console.warn('⚠️ Error sending stop signal to service:', error);
                                });
                            }
                            
                            // Clear global reference
                            window.cvEyeTracker = null;
                            console.log('✅ Eye tracking service shut down completely on course exit');
                        }
                    } catch (error) {
                        console.warn('⚠️ Error during course exit cleanup:', error);
                    }
                };
                
                // Listen for course exit events
                window.addEventListener('beforeunload', handleCourseExit);
                
                // Also listen for navigation to non-course pages
                ...
                history.pushState = function(...args) {
                    ...
                    setTimeout(() => {
                        ...
                        // If navigating away from module pages, trigger course exit
                        if (!currentUrl.includes('module_id=') && !currentUrl.includes('Smodulepart.php')) {
                            handleCourseExit();
                        }
                    }, 100);
                    return result;
