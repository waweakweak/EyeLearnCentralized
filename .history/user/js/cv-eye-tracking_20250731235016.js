...
class CVEyeTrackingSystem {
    ...
    async forceEmergencyShutdown() {
        ...
        try {
            // Create multiple parallel shutdown requests with different strategies
            const emergencyRequests = [];
            
            // Strategy 1: Rapid-fire stop_tracking calls (5 times)
            for (let i = 0; i < 5; i++) {
            ...
            // Strategy 2: Try health check to force service reset
            emergencyRequests.push(
                fetch(`${this.pythonServiceUrl}/api/health`, {
                    method: 'GET',
                    headers: { 'Content-Type': 'application/json' }
                }).catch(() => null)
            );
            
            // Execute all emergency requests
            await Promise.all(emergencyRequests);
            console.log('🚨 Emergency shutdown requests completed');
            
            // Wait for potential camera release
            await new Promise(resolve => setTimeout(resolve, 2000));
            
            // Final verification
            try {
                const finalCheck = await fetch(`${this.pythonServiceUrl}/api/frame`, {
                    method: 'GET',
                    headers: { 'Content-Type': 'application/json' }
                });
                ...
                if (finalCheck.ok) {
                    ...
                    if (data.hasFrame) {
                        console.warn('🚨 CRITICAL: Camera still active after emergency shutdown');
                        this.showPersistentCameraWarning();
                    } else {
                        console.log('✅ Emergency shutdown successful - camera released');
                    }
                } else {
                    console.log('✅ Service unreachable - emergency shutdown likely successful');
                }
            } catch (finalError) {
                console.log('✅ Service connection failed - camera likely released');
            }
