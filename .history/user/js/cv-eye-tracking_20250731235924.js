...
class CVEyeTrackingSystem {
    ...
    async forceEmergencyShutdown() {
        console.log('🚨 FORCE EMERGENCY CAMERA SHUTDOWN - Multiple aggressive attempts...');
        
        try {
            // Strategy 1: Create systematic shutdown sequence
            console.log('📹 Step 1: Systematic service shutdown...');
            
            // First, try to stop tracking properly with longer timeout
            for (let attempt = 1; attempt <= 3; attempt++) {
                try {
                    console.log(`🔄 Stop tracking attempt ${attempt}/3...`);
                    const controller = new AbortController();
                    ...
                    if (response.ok) {
                        const data = await response.json();
                        console.log(`✅ Stop tracking attempt ${attempt} successful:`, data.success);
                        
                        // Wait for camera to potentially release
                        await new Promise(resolve => setTimeout(resolve, 2000));
                        break;
                    }
                } catch (error) {
                    console.warn(`⚠️ Stop tracking attempt ${attempt} failed:`, error.message);
                    if (attempt < 3) {
            ...
            // Strategy 2: Try to force service reset through health checks
            console.log('📹 Step 2: Service reset attempts...');
            for (let i = 0; i < 3; i++) {
                try {
                    await fetch(`${this.pythonServiceUrl}/api/health`, {
                        method: 'GET',
                        headers: { 'Content-Type': 'application/json' }
                    });
                    await new Promise(resolve => setTimeout(resolve, 500));
                } catch (e) {
                    // Ignore errors, this is just to potentially reset the service
                }
            }
            
            // Strategy 3: Verify shutdown with extended monitoring
            console.log('� Step 3: Extended camera shutdown verification...');
            let cameraStillActive = false;
            
            for (let check = 1; check <= 5; check++) {
                try {
                    console.log(`🔍 Camera check ${check}/5...`);
                    
                    const controller = new AbortController();
                    const timeoutId = setTimeout(() => controller.abort(), 3000);
                    
                    const frameResponse = await fetch(`${this.pythonServiceUrl}/api/frame`, {
                        method: 'GET',
                        headers: { 'Content-Type': 'application/json' },
                        signal: controller.signal
                    });
                    ...
                    if (frameResponse.ok) {
                        ...
                        if (frameData.hasFrame && frameData.frameData) {
                            console.warn(`⚠️ Camera still active on check ${check}`);
                            cameraStillActive = true;
                            
                            // If still active, try additional stop attempts
                            await fetch(`${this.pythonServiceUrl}/api/stop_tracking`, {
                                method: 'POST',
                                headers: { 'Content-Type': 'application/json' }
                            }).catch(() => null);
                            ...
                            console.log(`✅ Camera inactive on check ${check} - no frame data`);
                            cameraStillActive = false;
                            break;
                        }
                    } else {
                        console.log(`✅ Camera service unresponsive on check ${check} - likely shut down`);
                        cameraStillActive = false;
                        break;
                    }
                } catch (checkError) {
                    console.log(`✅ Camera check ${check} failed - service likely down`);
                    cameraStillActive = false;
                    break;
                }
            }
            
            // Strategy 4: Final result and user notification
            if (cameraStillActive) {
                console.error('🚨 CRITICAL: Camera remains active after all shutdown attempts');
                this.showAdvancedCameraWarning();
            } else {
                console.log('✅ Camera successfully shut down after systematic attempts');
                this.showCameraSuccessNotification();
            }
    ...
    showPersistentCameraWarning() {
        const warning = document.createElement('div');
        warning.className = 'fixed top-4 right-4 bg-red-600 text-white px-6 py-4 rounded-lg shadow-lg z-50 max-w-sm';
        warning.innerHTML = `
            <div class="text-sm">
                <div class="font-semibold mb-2">⚠️ Camera Still Active</div>
                <div class="space-y-1 text-xs">
                    <div>📹 Python service may need manual restart</div>
                    <div>🔧 Check Task Manager for python.exe</div>
                    <div>🛑 Kill python.exe process if needed</div>
                </div>
                <button onclick="this.parentElement.remove()" class="mt-2 text-xs bg-red-500 hover:bg-red-700 px-2 py-1 rounded">
                    Dismiss
                </button>
            </div>
        `;
        ...
        // Don't auto-remove this warning - let user dismiss manually
    }

    showAdvancedCameraWarning() {
        const warning = document.createElement('div');
        warning.className = 'fixed top-4 right-4 bg-red-600 text-white px-6 py-4 rounded-lg shadow-lg z-50 max-w-md';
        warning.innerHTML = `
            <div class="text-sm">
                <div class="font-semibold mb-2">🚨 Camera Shutdown Failed</div>
                <div class="space-y-2 text-xs">
                    <div class="bg-red-700 p-2 rounded">
                        <div class="font-semibold">Manual Steps Required:</div>
                        <ol class="list-decimal list-inside mt-1 space-y-1">
                            <li>Press <kbd class="bg-red-800 px-1 rounded">Ctrl+Shift+Esc</kbd> to open Task Manager</li>
                            <li>Find <strong>python.exe</strong> processes</li>
                            <li>Right-click → <strong>End Task</strong></li>
                            <li>Camera light should turn off</li>
                        </ol>
                    </div>
                    <div class="text-yellow-200">
                        💡 Alternative: Restart the eye tracking service completely
                    </div>
                </div>
                <div class="flex gap-2 mt-3">
                    <button onclick="this.parentElement.remove()" class="text-xs bg-red-500 hover:bg-red-700 px-2 py-1 rounded">
                        Dismiss
                    </button>
                    <button onclick="window.open('taskmgr:///', '_blank')" class="text-xs bg-blue-500 hover:bg-blue-700 px-2 py-1 rounded">
                        Open Task Manager
                    </button>
                </div>
            </div>
    ...
    showCameraSuccessNotification() {
        const notification = document.createElement('div');
        notification.className = 'fixed top-4 right-4 bg-green-600 text-white px-6 py-4 rounded-lg shadow-lg z-50 max-w-sm';
        notification.innerHTML = `
            <div class="text-sm">
                <div class="font-semibold mb-2">✅ Camera Successfully Shut Down</div>
                <div class="space-y-1 text-xs">
                    <div>🔒 Camera access fully released</div>
                    <div>📹 Hardware camera turned off</div>
                    <div>💾 All data saved</div>
                </div>
            </div>
        `;
        document.body.appendChild(notification);
        ...
        try {
            ...
            if (this.isConnected) {
                try {
                    ...
                    // SYSTEMATIC CAMERA SHUTDOWN - Sequential attempts with proper verification
                    console.log('📹 Starting systematic camera shutdown sequence...');
                    
                    // Phase 1: Graceful shutdown with extended timeout
                    let shutdownSuccess = false;
                    
                    for (let attempt = 1; attempt <= 3; attempt++) {
                        try {
                            console.log(`📹 Graceful shutdown attempt ${attempt}/3...`);
                            ...
                            if (response.ok) {
                                const data = await response.json();
                                if (data.success) {
                                    console.log(`✅ Graceful shutdown attempt ${attempt} successful`);
                            ...
                            console.warn(`⚠️ Graceful shutdown attempt ${attempt} failed:`, error.message);
                            if (attempt < 3) {
                    ...
                    // Phase 2: Verification with multiple checks
                    console.log('📹 Verifying camera shutdown...');
                    let verificationPassed = false;
                    
                    for (let check = 1; check <= 3; check++) {
                        try {
                            console.log(`🔍 Verification check ${check}/3...`);
                            
                            const verifyResponse = await fetch(`${this.pythonServiceUrl}/api/frame`, {
                                method: 'GET',
                                headers: { 'Content-Type': 'application/json' }
                            });
                            
                            if (verifyResponse.ok) {
                                const frameData = await verifyResponse.json();
                                if (!frameData.hasFrame || !frameData.frameData) {
                                    console.log(`✅ Verification check ${check}: Camera successfully shut down`);
                                    verificationPassed = true;
                                    break;
                                } else {
                                    console.warn(`⚠️ Verification check ${check}: Camera still active`);
                                    
                                    // Additional stop attempt if camera still active
                                    await fetch(`${this.pythonServiceUrl}/api/stop_tracking`, {
                                        method: 'POST',
                                        headers: { 'Content-Type': 'application/json' }
                                    }).catch(() => null);
                                ...
                                console.log(`✅ Verification check ${check}: Service unreachable - likely shut down`);
                                verificationPassed = true;
                                break;
                            }
                            ...
                            console.log(`✅ Verification check ${check}: Service connection failed - camera likely released`);
                            verificationPassed = true;
                            break;
                        }
                    }
                    
                    // Phase 3: Final action based on verification
                    if (verificationPassed) {
                        console.log('✅ Camera shutdown completed successfully');
                        this.showCameraSuccessNotification();
                    } else {
                        console.warn('⚠️ Camera shutdown verification failed - triggering emergency shutdown');
                        await this.forceEmergencyShutdown();
                    }
                        // Attempt 1: Try shutdown endpoint (if it exists)
                        fetch(`${this.pythonServiceUrl}/api/shutdown`, {
                            method: 'POST',
                            headers: { 'Content-Type': 'application/json' }
                        }).catch(() => null),
                        
                        // Attempt 2: Try stop endpoint (if it exists)
                        fetch(`${this.pythonServiceUrl}/api/stop`, {
                            method: 'POST',
                            headers: { 'Content-Type': 'application/json' }
                        }).catch(() => null),
                        
                        // Attempt 3: Force stop tracking multiple times
                        fetch(`${this.pythonServiceUrl}/api/stop_tracking`, {
                            method: 'POST',
                            headers: { 'Content-Type': 'application/json' }
                        }).catch(() => null),
                        
                        // Attempt 4: Second stop tracking call
                        fetch(`${this.pythonServiceUrl}/api/stop_tracking`, {
                            method: 'POST',
                            headers: { 'Content-Type': 'application/json' }
                        }).catch(() => null)
                    ...
                    // Execute all shutdown attempts in parallel
                    console.log('📹 Executing multiple camera shutdown attempts...');
                    await Promise.all(shutdownAttempts);
                    console.log('📹 All camera shutdown attempts completed');
                    
                    // Wait a moment for camera to release
                    await new Promise(resolve => setTimeout(resolve, 1000));
                    
                    // Verify camera shutdown by trying to access it
                    try {
                        console.log('� Verifying camera shutdown...');
                        const verifyResponse = await fetch(`${this.pythonServiceUrl}/api/frame`, {
                            method: 'GET',
                        ...
                        if (verifyResponse.ok) {
                            ...
                            if (data.hasFrame) {
                                console.warn('⚠️ Camera still active after shutdown attempts');
                                // Try one more aggressive shutdown
                                await this.forceEmergencyShutdown();
                            } else {
                                console.log('✅ Camera successfully shut down - no more frames');
                            }
                        } else {
                            console.log('✅ Camera service appears to be shut down (no response)');
                        }
                    } catch (verifyError) {
                        console.log('✅ Camera service unreachable - likely shut down');
                    }
                    
                } catch (shutdownError) {
                    console.warn('⚠️ Error during camera shutdown process:', shutdownError);
                    
                    // Emergency fallback shutdown
                    await this.forceEmergencyShutdown();
                }
            }
        ...
        try {
            if (window.cvEyeTracker) {
                ...
                try {
                    ...
                    // AGGRESSIVE MULTI-STRATEGY SHUTDOWN
                    const emergencyShutdownRequests = [];
                    
                    // Strategy 1: Multiple stop_tracking calls
                    for (let i = 0; i < 5; i++) {
                        emergencyShutdownRequests.push(
                    ...
                    // Strategy 2: Try various potential endpoints
                    const potentialEndpoints = ['/api/shutdown', '/api/stop', '/api/pause'];
                    potentialEndpoints.forEach(endpoint => {
                        emergencyShutdownRequests.push(
                            fetch(`${pythonServiceUrl}${endpoint}`, {
                                method: 'POST',
                                headers: { 'Content-Type': 'application/json' }
                            }).catch(() => null)
                        );
                    ...
                    // Execute all emergency shutdown requests
                    await Promise.all(emergencyShutdownRequests);
                    console.log('📹 Emergency camera shutdown completed');
                    
                    // Verify shutdown with frame check
                    setTimeout(async () => {
                        try {
                            const verifyResponse = await fetch(`${pythonServiceUrl}/api/frame`);
                            if (verifyResponse.ok) {
                                const data = await verifyResponse.json();
                                if (data.hasFrame) {
                                    console.warn('⚠️ Camera still active after emergency shutdown');
                                } else {
                                    console.log('✅ Camera successfully shut down');
                                }
                            }
                        } catch (e) {
                            console.log('✅ Camera service unreachable - likely shut down');
            ...
            try {
                ...
                try {
                    ...
                    // ULTRA-AGGRESSIVE FINAL SHUTDOWN ATTEMPTS
                    const finalShutdownRequests = [];
                    
                    // Multiple rapid-fire stop_tracking calls
                    for (let i = 0; i < 10; i++) {
                        finalShutdownRequests.push(
                            fetch(`${pythonServiceUrl}/api/stop_tracking`, {
                                method: 'POST',
                                headers: { 'Content-Type': 'application/json' }
                            }).catch(() => null)
                    ...
                    const allEndpoints = ['/api/shutdown', '/api/stop', '/api/pause', '/api/health'];
                    ...
                    await Promise.all(finalShutdownRequests);
