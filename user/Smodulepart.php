    ...
    <div class="flex min-h-screen pt-16">
        ...
        <?php elseif ($selected_section): ?>
            <div class="max-w-4xl mx-auto">
                ...
                <!-- Completion button - always show if section not completed -->
                <?php if (!$is_section_completed): ?>
                <div class="mt-8 p-6 bg-yellow-50 border-l-4 border-yellow-400 rounded-lg">
                    <div class="flex items-center">
                        <div class="ml-3">
                            <h3 class="text-sm font-medium text-yellow-800">Ready to Continue?</h3>
                            <div class="mt-2 text-sm text-yellow-700">
                                <p>Click the button below to mark this section as complete<?php echo $selected_section['has_quiz'] ? ' and proceed to the quiz' : ''; ?>.</p>
                            </div>
                            <div class="mt-4">
                                <form method="post" id="completion-form">
                                    <input type="hidden" name="section_completed" value="1">
                                    <button type="submit" class="px-6 py-3 bg-blue-600 text-white rounded-md hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-blue-500">
                                        Mark as Complete <?php echo $selected_section['has_quiz'] ? '& View Quiz' : ''; ?>
                                    </button>
                                </form>
                            </div>
                        </div>
    ...
    if (currentSectionId) {
        ...
        .then(data => {
            if (data.success) {
                ...
                const progressText = document.querySelector('.text-xs.text-gray-700');
                ...
                // Update check mark for completed section
                ...
                window.location.href = nextUrl; // Navigate anyway if update fails
            }
        })
        .catch(error => {
            console.error('Error updating progress:', error);
            window.location.href = nextUrl; // Navigate anyway if there's an error
        });
    } else {
        window.location.href = nextUrl;
    }
